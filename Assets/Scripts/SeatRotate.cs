...
public class SeatRotate : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Mathf.Abs(ParentObject.transform.localRotation.eulerAngles.y-InitialRotation.eulerAngles.y)>=A && Mathf.Abs(ParentObject.transform.localRotation.eulerAngles.y - InitialRotation.eulerAngles.y) <= B)
        {
            ...
            ChildObject.transform.localRotation =Quaternion.Lerp(ChildObject.transform.localRotation, childObjTargetRotation,2f*Time.deltaTime);
        }
        else 
        {
            ChildObject.transform.localRotation = Quaternion.Lerp(ChildObject.transform.localRotation, childObjInitialRotation, 2f * Time.deltaTime); ;
        }
