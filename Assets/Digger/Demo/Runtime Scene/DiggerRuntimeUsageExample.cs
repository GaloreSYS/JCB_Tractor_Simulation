        ...
        private void Update()
        {
            // if (Input.GetMouseButton(0))
            {
                ...
                //if (Physics.Raycast(transform.position, transform.forward, out var hit, 2000f))
                {
                    ...
                    {
                        diggerMasterRuntime.ModifyAsyncBuffured(cubde.position, brush, action, textureIndex, opacity,
                            size);
                    }
                    else
                    {
                        diggerMasterRuntime.Modify(cubde.position, brush, action, textureIndex, opacity, size);
                    }
                }
            }
