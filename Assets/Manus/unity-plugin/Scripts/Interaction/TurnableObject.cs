...
namespace Manus.Interaction
{
    ...
    public class TurnableObject : MonoBehaviour, IGrabbable, IValue
    {
        ...
        public void OnAddedInteractingInfo(GrabbedObject p_Object, GrabbedObject.Info p_Info)
        {
            ...
            t_ExtraGrabInfo.handToObjectRotation = Quaternion.Inverse(p_Info.interactor.transform.rotation) *
        ...
        public void OnGrabbedFixedUpdate(GrabbedObject p_Object)
        {
            ...
            for (int i = 0; i < m_GrabInfo.Count; i++)
            {
                ...
                Quaternion t_Rot = Quaternion.Inverse(transform.rotation) * t_Info.interactor.transform.rotation;

                Vector3 t_Old = transform.InverseTransformDirection(transform.rotation * upAxis);
                Vector3 t_New = transform.InverseTransformDirection(t_Rot * upAxis);

                Vector2 t_OldProjDir = Project(t_Old);
                Vector2 t_NewProjDir = Project(t_New);
            ...
            for (int i = 0; i < m_GrabInfo.Count; i++)
            {
                ...
                m_GrabInfo[i].handToObjectRotation =
                    Quaternion.Inverse(m_GrabInfo[i].previousRotation) * transform.rotation;
            }
