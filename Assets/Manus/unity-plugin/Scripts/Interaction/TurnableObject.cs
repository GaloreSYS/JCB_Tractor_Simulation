...
namespace Manus.Interaction
{
    ...
    public class TurnableObject : MonoBehaviour, IGrabbable, IValue
    {
        ...
        /// <summary>
        /// Assigning a Center Point allows the object to rotate correctly even when moving through the world
        /// </summary>
        ...
        {
            onGrabEnd?.Invoke(this, p_Object);
        }
        ...
        public void OnAddedInteractingInfo(GrabbedObject p_Object, GrabbedObject.Info p_Info)
        {
            ...
            if (centerPoint)
            {
                t_ExtraGrabInfo.previousPosition = centerPoint.InverseTransformPoint(t_ExtraGrabInfo.previousPosition);
            }
        ...
        public void OnGrabbedFixedUpdate(GrabbedObject p_Object)
        {
            ...
            for (int i = 0; i < m_GrabInfo.Count; i++)
            {
                ...
                Vector3 t_PrevPos = t_Grab.previousPosition;
                if (centerPoint)
                {
                    t_PrevPos = centerPoint.TransformPoint(t_Grab.previousPosition);
                }
                Vector3 t_PrevDir = transform.InverseTransformPoint(t_PrevPos);
            ...
            for (int i = 0; i < m_GrabInfo.Count; i++)
            {
                ...
                if (centerPoint)
                {
                    m_GrabInfo[i].previousPosition = centerPoint.InverseTransformPoint(m_GrabInfo[i].previousPosition);
                }
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            if (centerPoint)
            {
                ...
                Gizmos.DrawSphere(centerPoint.position, 0.01f);
            }
            Gizmos.color = t_Col;
        }
    }
